...
angular.module('eayun.price')
    .controller('ConfigCtrl', ['$scope','SysCode','eayunModal','toast','item','payType','priceService',
        ...
        vm.params = {
            dcId : vm.item.dcId,
            payType : payType,
            billingFactor : vm.item.billingFactor,
            resourcesType : vm.item.resourcesType,
            billingUnit : vm.item.factorUnit
        }
        vm.getList = function(){
            vm.isAdd = false;
            vm.isEdit = false;
        ...
        vm.deletePrice = function (id) {
            eayunModal.confirm('确定要删除计费区间吗？').then(function () {
                priceService.deleteFactorPrice({'id':id,'factorName':vm.item.factorName,'payType':payType}).then(function (data) {
                    if(data.respCode == SysCode.success){
        ...
        vm.editPrice = function (priceItem) {
            ...
            vm.editItem.factorName = vm.item.factorName;
